...
<div class="bg-white rounded-xl shadow-md border border-gray-200 p-6 mb-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Subcategorías - {{ $class->code }} {{ $class->name }}</h2>
            <p class="text-gray-600">Administrar subcategorías de la clase</p>
        </div>
        <div class="mt-4 md:mt-0 flex flex-col sm:flex-row gap-3">
            <a href="{{ route('admin.subcategories.create', $class) }}" class="bg-idera-blue text-white px-6 py-2 rounded-lg hover:bg-opacity-90 transition font-medium">
                + Nueva Subcategoría
            </a>
            ...
            <a href="{{ route('dashboard') }}" class="px-4 py-2 bg-gray-100 text-gray-800 rounded-lg hover:bg-gray-200 transition font-medium">← Dashboard</a>
        </div>
    </div>
    ...
    <div class="flex flex-wrap gap-3 mb-6">
        ...
        @if(request('trashed'))
            <a href="{{ route('admin.classes.show', $class) }}" class="px-4 py-2 bg-red-100 text-red-800 rounded-lg hover:bg-red-200 transition">Limpiar Filtros</a>
        @endif
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                        ...
                        @if($subcategory->trashed())
                            <form method="POST" action="{{ route('admin.subcategories.restore', $subcategory->id) }}" class="inline">
                                @csrf @method('POST')
                                <button type="submit" class="text-green-600 hover:text-green-500 font-medium" onclick="return confirm('¿Restaurar esta subcategoría?')">Restaurar</button>
                            </form>
                        ...
                        @else
                            <form method="POST" action="{{ route('admin.subcategories.destroy', $subcategory) }}" class="inline">
                                @csrf @method('DELETE')
                                <button type="submit" class="text-red-600 hover:text-red-500 font-medium" onclick="return confirm('¿Hacer invisible esta subcategoría?')">Hacer Invisible</button>
                            </form>
                        @endif
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                        ...
                        <p class="mt-2 text-lg">No hay subcategorías en esta clase</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
