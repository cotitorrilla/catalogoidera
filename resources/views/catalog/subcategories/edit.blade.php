...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-md border border-gray-200 p-8">
        <h2 class="text-2xl font-bold text-gray-800 mb-8">Editar Subcategoría</h2>
        ...
        <form method="POST" action="{{ route('admin.subcategories.update', $subcategory) }}">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <select name="class_id" required class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-idera-blue focus:border-transparent @error('class_id') border-red-500 @enderror">
                        ...
                        @foreach($classes as $class)
                            <option value="{{ $class->id }}" {{ old('class_id', $subcategory->class_id) == $class->id ? 'selected' : '' }}>{{ $class->code }} - {{ $class->name }}</option>
                        @endforeach
                    </select>
                    @error('class_id')
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $subcategory->code) }}" required readonly 
                ...
                <div>
                    ...
                    <textarea name="definition" rows="4" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-idera-blue focus:border-transparent @error('definition') border-red-500 @enderror">{{ old('definition', $subcategory->definition) }}</textarea>
                    @error('definition')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
